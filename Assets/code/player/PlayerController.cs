...
public class PlayerController : MonoBehaviour
{
    ...
    */
    ...
    Vector3 GetWallNormals()
    {
        ...
        return -mas.normal.GetNormalFromListOfColliders(wallCols);
    }
    ...
    bool shouldWallJump()
    {
        ...
        if (cols.Length > 2)
        {
            foreach (Collider col in cols)
            {
                ...
                {
                    // If the player is able to wall run
                    if (canWallJump && s == state.walking && !grounded() && magic.key.down(keys.jump))
                    {
                        return true;
                    }
                }
    ...
    bool shouldDash => canDash && magic.key.down(keys.dash) && !dashing && stamina - staminaPerDash >= 0;
    ...
    */
    ...
    void Update()
    {
        if (s != state.dead)
        {
            ...
            if (magic.key.down(keys.slide) && s != state.sliding && grounded())
            {
                Slide();
            }

            if (magic.key.down(keys.noclip))
            {
                if (aS == AdminState.standard)
                {
                    aS = AdminState.noclip;
                }
                else
                {
                    aS = AdminState.standard;
                }
            }

            if (magic.key.up(keys.slide) && s == state.sliding)
            {
                Unslide();
            }

            if (magic.key.down(keys.slam) && !grounded() && s != state.slamming)
            {
                Slam();
            }
        ...
        else
        {
            ...
            {
                StopCoroutine(mas.player.GetCombatController().slowTimeCR);
                Time.timeScale = 1;
        ...
        if (magic.key.down(keys.goToSpawnKey))
        {
    ...
    IEnumerator LerpVelocity(float t, Vector3 currentVector, Vector3 TargetVector)
    {
        ...
        if (t < 1)
        {
            newVector = mas.vector.LerpVectors(currentVector, TargetVector, t);
        }
